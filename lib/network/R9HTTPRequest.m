...
- (id)initWithURL:(NSURL *)targetUrl
{
    ...
    if (self) {
        ...
        NSString *token = [XMDataManager sessionCookie];
        if(token){
            [self addHeader:token forKey:SESSION_NAME];
        }
    }
    return self;
}
...
//        [alert show];
        ...
        if (self.failedHandler) {
            NSDictionary *info = @{@"title":@"通信エラー",@"message":@"通信が可能な状態か確認してください。"};
            NSError *err = [NSError errorWithCode:kErrorCodeNoConnection userInfo:info];
            self.failedHandler(err);
        }
        return NO;
    }
    else {
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    [self setCompletionBlock:^{
        ...
        [queue addOperationWithBlock:^{
            if (self.failedHandler) {
                NSDictionary *info = @{@"title":@"通信エラー", @"message":@"通信エラーが発生しました"};
                NSError *err = [NSError errorWithCode:kErrorCodeConnectionError userInfo:info];
                self.failedHandler(err);
            }
        }];
    }];
